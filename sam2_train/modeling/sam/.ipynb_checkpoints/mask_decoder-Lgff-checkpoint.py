...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        # ======================== 替换为：置信度门控的残差 LGFF ========================
        original_dtype = src.dtype  # 移到条件判断外面，避免作用域问题
        if self.use_lgff:
            self.lgff_final_refiner.to(src.device)

            # 用 autocast 兼容半精度；LGFF 内部若不支持半精度，外部转 float32 计算
            use_amp = original_dtype in (torch.float16, torch.bfloat16)
            with torch.cuda.amp.autocast(enabled=use_amp):
                refined = self.lgff_final_refiner(g=src.float(), x=src.float())
            refined = refined.to(dtype=original_dtype)

            # 从 IoU token 特征得到门控 α (B, C)->(B,1,1,1)
            #alpha = self.gate_mlp(iou_token_out).view(-1, 1, 1, 1)
            alpha = self.gate_mlp(iou_token_out.float()).view(-1, 1, 1, 1)
            alpha = alpha.to(dtype=src.dtype)

            
            # 残差融合（核心）：src ← src + α * (refined − src)
            src = src + alpha * (refined - src)
        # ======================== 残差 LGFF 结束 ========================
        # 这里开始改：
        use_amp = original_dtype in (torch.float16, torch.bfloat16)
...
# （可选）如果你希望保持和 src 一致的 dtype：
# iou_pred = iou_pred.to(dtype=src.dtype)
